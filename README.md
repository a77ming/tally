...
<img src="assets/screenshot-apps.png" width="340" alt="Tally — Claude Code vs Codex with quota">&nbsp;&nbsp;<img src="assets/screenshot-providers.png" width="340" alt="Tally — per-provider usage via cc-switch">

</div>

---

Tally is a 100% native SwiftUI menu bar app for macOS (14+, Apple Silicon & Intel) that shows what your Claude Code and Codex CLI sessions are actually costing you — at a glance, without leaving your desk or running a CLI. Fully local: zero network calls, no accounts, no telemetry.
...
- **Codex CLI too** — Tally also reads Codex's session rollouts: GPT model usage, estimated cost, and your live rate-limit quota (5-hour and weekly windows).
- **Four breakdowns**:
  - **Apps** — Claude Code vs Codex side by side, with Codex's remaining quota inline.
...
| `~/.codex/sessions/**/*.jsonl` | Codex CLI's session rollouts: GPT tokens (incl. cached input), model, project, rate-limit quota | Optional |
...
No. It only parses Claude Code's and Codex's `.jsonl` session logs and (optionally) cc-switch's usage and pricing tables. Keys, prompts, and responses are never touched.
...
Tally 是一款 100% 原生 SwiftUI 的 macOS 菜单栏应用（macOS 14+，Apple Silicon 与 Intel 均支持），让你一眼看清 Claude Code 和 Codex CLI 到底花了多少钱——不用开终端、不用跑 CLI。完全本地运行：零网络请求、无账号、无遥测。
...
- **Codex CLI 同样支持** — Tally 也会读取 Codex 的会话日志：GPT 系列模型用量、估算费用，以及实时的限额配额（5 小时窗口与每周窗口）。
- **四种维度拆分**：
  - **应用** — Claude Code 与 Codex 并列对比，Codex 剩余配额内联显示。
...
| `~/.codex/sessions/**/*.jsonl` | Codex CLI 的会话日志：GPT token（含缓存输入）、模型、项目、限额配额 | 可选 |
...
不会。它只解析 Claude Code 与 Codex 的 `.jsonl` 会话日志和（可选的）cc-switch 用量与定价表。Key、提示词和模型回复都不会被读取。
